...
5. **HMAC rollout** :
   ...
   - **Phase 1** : déployer backend avec `OWSHmac:Enabled=true, RequireSignature=false`. Patcher les clients. Surveiller les logs Serilog `"HMAC opt-in: unsigned request accepted"` jusqu'à ce qu'ils disparaissent.
   - **Phase 2** : flipper `OWSHmac:RequireSignature=true`. Les requêtes non-signées retournent désormais 401.
